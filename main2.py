...
class TimelineHeatmap(QWidget):
    ...
    def _get_date_at_pos(self, pos) -> Optional[QDate]:
        ...
        if avail_w <= 0 or avail_h <= 0: return None
        ...
        if 0 <= col < 53 and 0 <= row < 7:
            ...
            if target <= self.today: return target
        return None
    ...
    def paintEvent(self, event):
        ...
        for i in range(53 * 7):
            ...
            if current_date > self.today: break
            col, row = i // 7, i % 7
            ...
            pen = QPen(Qt.NoPen)
            if current_date == self.selected_date: pen = QPen(QColor(236, 239, 244, 255), max(1, cell_size * 0.1))
...
class SettingsDialog(QDialog):
    def __init__(self, current_config: dict, parent=None):
        ...
        lbl1 = QLabel("Timeout:"); lbl1.setStyleSheet("color: #81A1C1; font-size: 13px; background: transparent;")
        lbl2 = QLabel("Unit:"); lbl2.setStyleSheet("color: #81A1C1; font-size: 13px; background: transparent;")
        ...
        btn_style = "QPushButton { background: rgba(255, 255, 255, 0.05); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 8px; padding: 10px; color: #ECEFF4; font-weight: bold; text-transform: uppercase; } QPushButton:hover { background: rgba(136, 192, 208, 0.2); border-color: #88C0D0; color: #88C0D0; }"
        self.btn_cancel.setStyleSheet(btn_style); self.btn_save.setStyleSheet(btn_style)
        self.btn_cancel.clicked.connect(self.reject); self.btn_save.clicked.connect(self.accept)
...
class EditTagsDialog(QDialog):
    def __init__(self, current_tags: List[str], parent=None):
        ...
        self.listw.setStyleSheet(f"""
            ...
            {SCROLLBAR_CSS}
        """)
        for t in self.final_tags: self.listw.addItem(t)
        ...
        for b in (btn_add, btn_edit, btn_remove): b.setStyleSheet(btn_style)
        controls_layout.addWidget(btn_add); controls_layout.addWidget(btn_edit); controls_layout.addWidget(btn_remove)
        ...
        def on_edit():
            if self.listw.currentItem():
                ...
                if dlg.exec() == QDialog.Accepted and (t := dlg.get_text().strip()): self.listw.currentItem().setText(t)
        def on_remove():
            if self.listw.currentItem(): self.listw.takeItem(self.listw.row(self.listw.currentItem()))

        btn_add.clicked.connect(on_add); btn_edit.clicked.connect(on_edit); btn_remove.clicked.connect(on_remove)

        btn_layout = QHBoxLayout()
        btn_layout.setContentsMargins(0, 15, 0, 0)
        btn_cancel = QPushButton("Cancel")
        btn_save = QPushButton("Save Tags")
        btn_cancel.setStyleSheet(btn_style); btn_save.setStyleSheet(btn_style.replace("#B48EAD", "#88C0D0"))
        btn_cancel.clicked.connect(self.reject)
        ...
        btn_layout.addWidget(btn_cancel); btn_layout.addWidget(btn_save)
        panel_layout.addLayout(btn_layout)
        layout.addWidget(self.panel)
...
class EntryCard(QWidget):
    def __init__(self, title: str, updated: str, tags: Optional[List[str]] = None, category_name: Optional[str] = None):
        ...
        tfont = QFont(); tfont.setBold(True); tfont.setPointSize(11)
...
class DiaryApp(QMainWindow):
    ...
    def _build_ui(self):
        ...
        editor_scroll, preview_scroll = self.text_editor.verticalScrollBar(), self.preview.verticalScrollBar()
        ...
        row1.setSpacing(10); row2.setSpacing(10)
        ...
        row1.addWidget(self.stat_entries); row1.addWidget(self.stat_words)
        row2.addWidget(self.stat_tags); row2.addWidget(self.stat_streak)
        right_stats_layout.addLayout(row1); right_stats_layout.addLayout(row2)
    ...
    def _sync_scroll_to_preview(self):
        if getattr(self, '_syncing_scroll', False): return 
        e_scroll, p_scroll = self.text_editor.verticalScrollBar(), self.preview.verticalScrollBar()
        if e_scroll.maximum() <= 0 or p_scroll.maximum() <= 0: return
    ...
    def _build_toolbar(self):
        ...
        for a in all_actions:
            if a not in (self.act_ribbon, act_search): self.tb.addAction(a)
            self.addAction(a) 
    ...
    def lock_app(self):
        ...
        while True:
            ...
            if unlock_screen.exec() != QDialog.Accepted: sys.exit(0)
            try:
                with open(SALT_FILE, "rb") as f: salt = f.read()
                with open(VERIFY_FILE, "rb") as f: token = f.read()
                if CryptoManager(unlock_screen.get_password(), salt).decrypt(token) == b"DIARY_VERIFIED": break
            except Exception:
    ...
    def update_preview(self):
        ...
        if self.app_config.get("show_stats", True):
            ...
            if words == 0: time_str = "0 min read"
            elif words < 100: time_str = "< 1 min read"
            else: time_str = f"{math.ceil(words / 200)} min read"
    ...
    def _delete_tag_globally(self, tag: str):
        if QMessageBox.question(self, "Confirm Global Delete", f"Permanently delete the tag '{tag}' from all entries?") == QMessageBox.Yes:
            if tag in self.active_tags: self.active_tags.remove(tag)
            try:
                if hasattr(self.db, "delete_tag"): self.db.delete_tag(tag)
                for db_row in self.db.get_entries():
                    r = dict(db_row)
                    tags = self.db.get_entry_tags(r["id"]) if hasattr(self.db, "get_entry_tags") else (r.get("tags") or [])
                    if tag in tags:
                        tags.remove(tag)
                        if hasattr(self.db, "set_entry_tags"): self.db.set_entry_tags(r["id"], tags)
                        else:
                            imgs = self.db.get_entry_images(r["id"]) if hasattr(self.db, "get_entry_images") else []
                            try: self.db.update_entry(r["id"], r.get("title", ""), r.get("content", b""), tags, imgs, update_timestamp=False)
                            except TypeError: self.db.update_entry(r["id"], r.get("title", ""), r.get("content", b""), tags, imgs)
            except Exception: pass
            self.load_tags()
            self.load_entries(keep_selection=True)
    ...
    def _create_entry_item(self, row: dict, tags: List[str], category_name: Optional[str] = None, target_list: QListWidget = None) -> QListWidgetItem:
        ...
        try: display_date = datetime.strptime(str(raw_date).split(".")[0].replace("T", " ").replace("Z", ""), "%Y-%m-%d %H:%M:%S").strftime("%b %d, %Y %I:%M %p")
        except Exception: display_date = str(raw_date)[:16] or "—"
    ...
    def _preserve_selection_and_scroll(self):
        return (self.entry_list.currentItem().data(Qt.UserRole) if self.entry_list.currentItem() else None, self.entry_list.verticalScrollBar().value())
    ...
    def load_entries(self, keep_selection: bool = True, search_text: Optional[str] = None):
        ...
        for r in rows:
            try: cat_name = self.db.get_category(r.get("category_id") or r.get("category"))[1] if hasattr(self.db, "get_category") else r.get("category")
            except Exception: cat_name = r.get("category")
        ...
        for r in self.entries_cache: self._create_entry_item(r, r["__tags"], r["__category_name"])
    ...
    def open_dashboard(self): self.right_stack.setCurrentIndex(2)

    def update_dashboards(self):
        self.timeline_widget.set_data(self.entries_cache)
        for d in self.formatted_dates: self.calendar_widget.setDateTextFormat(d, QTextCharFormat())
        ...
        for r in self.entries_cache:
            try: total_words += len(self.crypto.decrypt(r["content"]).decode(errors="ignore").split())
            except Exception: pass
        ...
        if active_dates:
            ...
            if sorted_dates[0] in (today, today - timedelta(days=1)):
                ...
                for i in range(1, len(sorted_dates)):
                    if (sorted_dates[i-1] - sorted_dates[i]).days == 1: streak += 1
                    else: break
        ...
        for qdate, count in day_counts.items():
            ...
            fmt.setForeground(QColor(255, 255, 255)); fmt.setFontWeight(QFont.Bold)
    ...
    def _on_dashboard_date_clicked(self, date: QDate):
        ...
        for r in self.entries_cache:
            if d_str := (r.get("created_at") or r.get("updated_at")):
                try:
                    dt = datetime.strptime(str(d_str).split(".")[0].replace("T", " ").replace("Z", ""), "%Y-%m-%d %H:%M:%S")
                    if dt.year == date.year() and dt.month == date.month() and dt.day == date.day(): matched.append(r)
                except Exception: pass
                    
        self.dash_date_label.setText(f"{len(matched)} Entries on {date.toString('MMMM d, yyyy')}")
        for r in matched: self._create_entry_item(r, r.get("__tags", []), r.get("__category_name"), self.dash_entry_list)

    def _on_dash_list_item_clicked(self, item: QListWidgetItem):
        self._select_in_list(item.data(Qt.UserRole))
        if self.entry_list.currentItem(): self._on_list_item_clicked(self.entry_list.currentItem())
    ...
    def save_entry(self):
        ...
        for uid in orphans:
            self.entry_images.pop(uid, None) # Removed from active entry memory, but kept on disk.
        ...
        if self.current_entry and getattr(self.current_entry, "id", None):
            if content != getattr(self.current_entry, "content", "") or title != getattr(self.current_entry, "title", "") or [u for u, _ in images_data] != [u for u, _ in getattr(self.current_entry, "images", [])]:
                try: self.db.update_entry(self.current_entry.id, title, enc_content, tags, images_data, update_timestamp=True)
                except TypeError: self.db.update_entry(self.current_entry.id, title, enc_content, tags, images_data)
        ...
        if getattr(self.current_entry, "id", None): self._select_in_list(self.current_entry.id)
        self.show_status("")
    ...
    def delete_entry(self):
        ...
        if QMessageBox.question(self, "Confirm", "Delete entry?") == QMessageBox.Yes:
            ...
            # Disabled physical image deletion from disk
            # for uid, _ in getattr(self.current_entry, "images", []):
            #     try: os.remove(os.path.join(IMAGE_FOLDER, uid))
            #     except Exception: pass
            ...
            self.current_entry = None
            self.entry_images.clear(); self.text_editor.clear(); self.preview.clear()
            self.right_stack.setCurrentIndex(0)
            self.load_entries(keep_selection=False)

    def add_image(self):
        if not self.current_entry: return QMessageBox.warning(self, "No Entry", "Create or select an entry")
        if path := QFileDialog.getOpenFileName(self, "Select Image")[0]:
            try:
                ...
                self.update_preview()
            except Exception: QMessageBox.warning(self, "Error", "Failed to add image")

    def add_tag(self):
        if not self.current_entry or not getattr(self.current_entry, "id", None): return QMessageBox.warning(self, "No Entry", "Create or select an entry")
    ...
    def _persist_tags(self, entry: Entry):
        ...
        try:
            if hasattr(self.db, "set_entry_tags"): self.db.set_entry_tags(entry.id, entry.tags)
                ...
                try: self.db.update_entry(entry.id, getattr(entry, "title", "Untitled"), self.crypto.encrypt(getattr(entry, "content", "").encode()), entry.tags, [(u, n) for u, (n, _, _) in getattr(entry, "images", [])], update_timestamp=False)
                except TypeError: self.db.update_entry(entry.id, getattr(entry, "title", "Untitled"), self.crypto.encrypt(getattr(entry, "content", "").encode()), entry.tags, [(u, n) for u, (n, _, _) in getattr(entry, "images", [])])
        except Exception: pass

    def edit_tags_dialog(self):
        if not self.current_entry: return QMessageBox.warning(self, "No Entry", "Select an entry")
    ...
    def _show_entry_context_menu(self, pos):
        menu = QMenu(self)
        menu.addAction("New Entry", self.new_entry); menu.addAction("Save Entry", self.save_entry); menu.addAction("Delete Entry", self.delete_entry); menu.addSeparator()
        ...
        if not cats: move_menu.addAction("(no folders)").setEnabled(False)
        else:
            for cid, name in cats: move_menu.addAction(name).triggered.connect(lambda checked=False, _c=cid: self._move_selected_entry_to_category(_c))
        menu.addSeparator(); menu.addAction("Refresh", self.refresh_entries); menu.addAction("Toggle Sort", self.toggle_sort)
    ...
    def _move_selected_entry_to_category(self, category_id: int):
        if self.entry_list.currentItem() and self.current_entry:
            try: self.db.update_entry_category(self.entry_list.currentItem().data(Qt.UserRole), category_id); self.load_entries(keep_selection=True)
            except Exception: pass

    def _show_editor_context_menu(self, pos):
        menu = QMenu(self)
        menu.addAction("Add Image", self.add_image); menu.addAction("Add Tag", self.add_tag); menu.addAction("Edit Tags…", self.edit_tags_dialog)
        fm = menu.addMenu("Format")
        fm.addAction("Markdown", lambda: self._set_format_for_current("markdown")); fm.addAction("Text", lambda: self._set_format_for_current("text"))
        menu.addSeparator(); menu.addAction("Save Entry", self.save_entry)
        menu.exec(self.text_editor.mapToGlobal(pos))

    def _set_format_for_current(self, fmt: str):
        if not self.current_entry: self.current_entry = Entry(None, None, "", None, None, [], [], None)
        setattr(self.current_entry, "format", fmt); self.update_preview()
    ...
    def _clear_tag_filters(self):
        self.active_tags.clear(); self.load_entries(keep_selection=True)
...
def main():
    ...
    if unlock_screen.exec() != QDialog.Accepted: sys.exit(0)
    if not (pw := unlock_screen.get_password()): sys.exit(0)
    ...
    if not os.path.exists(SALT_FILE):
        try:
            with open(SALT_FILE, "wb") as f: f.write(crypto.salt)
        except Exception: pass

    if not os.path.exists(VERIFY_FILE):
        try:
            if entries := list(db.get_entries()): crypto.decrypt(entries[0]["content"])
        except Exception:
            QMessageBox.critical(None, "Denied", "Incorrect master password for existing data!")
            sys.exit(1)
        with open(VERIFY_FILE, "wb") as f: f.write(crypto.encrypt(b"DIARY_VERIFIED"))
    else:
        try:
            with open(VERIFY_FILE, "rb") as f:
                if crypto.decrypt(f.read()) != b"DIARY_VERIFIED": raise Exception
        except Exception:
